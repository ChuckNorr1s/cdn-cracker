...
command = f"sudo masscan -p80,443 --range {network_range} --rate=1000 --banners --max-rate 10000 --script=banner --wait 0 -oG - | grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" | sed '/\b\(25[0-5]\|2[0-4][0-9]\|[01]\?[0-9][0-9]\?\)\(\.\(25[0-5]\|2[0-4][0-9]\|[01]\?[0-9][0-9]\?\)\)\{3\}\b/!d' > ips.txt"
